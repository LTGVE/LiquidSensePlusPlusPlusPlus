...
@ModuleInfo(name = "AutoPlay",  category = ModuleCategory.PLAYER,description = "自动GG")
class AutoPlay : Module() {
    private var clickState = 0
    private val autogg = BoolValue("AutoGG", true)
    private val Clientname = TextValue("Clientname","MoLuSense")
                ...
                "huayuting" -> {
                    if (text.contains("      喜欢      一般      不喜欢", true)) {
                        if (autogg.get()){
                            ...
                            mc.thePlayer!!.sendChatMessage("["+Clientname.get()+"]Good Game !")
                        }
